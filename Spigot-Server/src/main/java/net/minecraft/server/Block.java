...
public class Block {
    ...
    public void dropsItems(World world, EntityHuman entityhuman, BlockPosition blockposition, IBlockData iblockdata, TileEntity tileentity, Collection<org.bukkit.inventory.ItemStack> collection, boolean dropOnFeet, boolean same) {
    	if (same)
    	{
    		if (dropOnFeet)
    			this.a(world, new BlockPosition(entityhuman.locX, entityhuman.locY, entityhuman.locZ), iblockdata);
    		else
        ...
        if (this.I() && EnchantmentManager.hasSilkTouchEnchantment(entityhuman)) {
            ...
            int i = EnchantmentManager.getBonusBlockLootEnchantmentLevel(entityhuman);
          if (!dropOnFeet)
        	  this.dropsItemsInCollection(world, blockposition, iblockdata, i, collection);
          else
        	  this.dropsItemsInCollection(world, new BlockPosition(entityhuman.locX, entityhuman.locY, entityhuman.locZ), iblockdata, i, collection);

//          this.b(world, blockposition, iblockdata, i);
        }
    }

    public final void dropsItemsInCollection(World world, BlockPosition blockposition, IBlockData iblockdata, int i, Collection<org.bukkit.inventory.ItemStack> collection) {
    	for (org.bukkit.inventory.ItemStack stack : collection)
    	{
    		this.dropNaturallyIdentified(world, blockposition, iblockdata, 1.0F, i, CraftItemStack.asNMSCopy(stack), stack);
    	}

    }

    public void dropNaturallyIdentified(World world, BlockPosition blockposition, IBlockData iblockdata, float f, int i, ItemStack stack, org.bukkit.inventory.ItemStack count) {
        if (!world.isClientSide) {
            int j = this.getDropCount(i, world.random);
            ...
            for (int k = 0; k < j; ++k) {
                // CraftBukkit - <= to < to allow for plugins to completely disable block drops from explosions
                if (world.random.nextFloat() < f) {
                    a(world, blockposition, new ItemStack(stack.getItem(), count.getAmount(), stack.getData()));
                }
            }
        }
    }
